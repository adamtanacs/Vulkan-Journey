...
void HelloTriangleApp::createGraphicsPipeline()
{
	...
	// Define depth testing in pipeline
	VkPipelineDepthStencilStateCreateInfo depthStencil{};
	depthStencil.sType				= VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
	depthStencil.depthTestEnable		= VK_TRUE; /* Compare depth values, 
													  discard fragments */
	depthStencil.depthWriteEnable		= VK_TRUE; /* Write passed depth values */
	// Define depth test comparison
	depthStencil.depthCompareOp			= VK_COMPARE_OP_LESS;
	// Test if depth values are within range
...
void HelloTriangleApp::createRenderPass()
{
	...
	// Don't need after rendering
	depthAttachment.storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
	// We don't use stencil now
...
void HelloTriangleApp::createFramebuffers()
{
	...
	for (size_t i = 0; i < swapChainImageViews.size(); ++i)
	{
		...
		std::array<VkImageView,2> attachments = {
			...
			depthImageView /* same depth image can be used per frame buffer */
		};
...
void HelloTriangleApp::oldCreateVertexBuffer()
{
	...
	// Define our buffers properties
	VkBufferCreateInfo bufferInfo{};
	bufferInfo.sType		= VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
	// Define size of our buffer (in bytes)
	bufferInfo.size			= sizeof(vertices[0]) * vertices.size();
	// Define how we want to use our buffer
	// a buffer can have multiple use, defined by bitwise or
	bufferInfo.usage		= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
	// Define ownership of our buffer (exactly like images in swap chain)
	bufferInfo.sharingMode	= VK_SHARING_MODE_EXCLUSIVE;
	// Flags is used to define sparse buffer memory
	// (spares = consists of multiple allocated partitions)
	// ------------------------------- 256 kb buffer -------------------------------
	// - VkDevice Memory A -|------- VkDevice Memory B -------|- VkDevice Memory C -
	// --- 64 KB S.Block ---|- 64 KB S.Block - 64 KB S.Block -|--- 64 KB S.Block ---
	bufferInfo.flags		= 0;
	...
	// First we need to query the memory requirements
	VkMemoryRequirements memRequirements;
	vkGetBufferMemoryRequirements(device, vertexBuffer, &memRequirements);

	// Define the properties for VRAM allocation
	VkMemoryAllocateInfo allocInfo{};
	allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	// Define the size of our allocated memory
	allocInfo.allocationSize = memRequirements.size;
	// Define the memory we want to use by giving its index
	// We check for:
	// 1. The type of memory we need
	// 2. The properties we want the memory to have
	allocInfo.memoryTypeIndex = findMemoryType(memRequirements.memoryTypeBits,
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
		// make memory data match the host memory 
		// HOWEVER at the cost of (not significant) performance
	...
	vkBindBufferMemory(device, vertexBuffer, vertexBufferMemory, 
		0 /* offset of our memory, if non-zero use memRequirements.alignment */
	);

	// We fill up our buffer with (vertex) data
	void* data;
	// Size can be set to VK_WHOLE_SIZE to map all of the memory
	vkMapMemory(device, vertexBufferMemory, 0, bufferInfo.size, 0, &data);
	// Other option to make memory "coherent"
	// Use vkInvalidateMappedMemoryRanges before reading from deviceMemory
	memcpy(data, vertices.data(), (size_t)bufferInfo.size);
	// Use vkFlushMappedMemoryRanges after writing to deviceMemory
...
{
	// Format: we only need a reasonable accuracy (at least 24 bits)
	// Usable formats
...
void HelloTriangleApp::createTextureImage()
{
	...
	// Create staging buffer for our image
	VkBuffer stagingBuffer;
	VkDeviceMemory stagingBufferMemory;
	...
	// Transition our image into transfer dst optimal layout
	transitionImageLayout(
		textureImage,
	...
	// Copy our buffer data into the image object
	copyBufferToImage(
		stagingBuffer,
		textureImage,
	...
	// Transition our image into shader READ-ONLY optimal layout
	transitionImageLayout(
		textureImage,
		VK_FORMAT_R8G8B8_SRGB,
		VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
VkFormat HelloTriangleApp::findSupportedFormat(
...
)
{
	...
	{
		// List all the properties for our format (which are the following)
		// linearTilingFeatures		: use cases supported with linear tiling
		// optimalTilingFeatures	: use cases supported with optimal tiling
		// bufferFeatures			: use cases supported with buffers
...
VkFormat HelloTriangleApp::findDepthFormat()
{
	return findSupportedFormat(
		{VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT},
		VK_IMAGE_TILING_OPTIMAL,
		VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT /* Important to use:
													      VK_FORMAT_FEATURE_ flag 
														  NOT
														  VK_IMAGE_USAGE_ flag*/
	);
}
...
uint32_t HelloTriangleApp::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
	...
	// This struct contains to arrays
	// memoryTypes	: type of the memory heap in question
	// memoryHeaps	: deducated VRAM or swap space in RAM for when VRAM runs out
	// (here we only focus on the type, 
	//  but different heaps can have different performances)
	vkGetPhysicalDeviceMemoryProperties(physicalDevice, &memProperties);

	for (uint32_t i = 0; i < memProperties.memoryTypeCount; i++)
	{
		if (typeFilter & (1 << i) && /* Check for a suitable memory type*/
			// Check if our memory also has the required properties for us
			// (Example: map it to CPU or write to it)
...
)
{
	...
	region.bufferRowLength = 0;		/* Specify size of rows, could include padding */
	region.bufferImageHeight = 0;	/* Specify size of columns, could include padding */
	...
	vkCmdCopyBufferToImage(
		...
		buffer,
		image,
		VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, /* current layout our image uses */
		1,
		&region
	);
...
)
{
	...
	// is using an image memory barrier (pipeline barrier)
	// - to ensure synchronization to resources
	// - to transition image layouts (like here)
	// - to transfer queue family ownership when VK_SHARING_MODE_EXCLUSIVE is used
	VkImageMemoryBarrier barrier{};
	barrier.sType		= VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
	// We define the layout transition
	barrier.oldLayout			= oldLayout;
	barrier.newLayout			= newLayout;
	// We define the queue family transition
	barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED; /* we ignore them for now */
	barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED; /* we ignore them for now */
	barrier.image				= image;
	// Here we define our image that is affected by the transitions
	...
	// Define where we want to read or write with our image
	barrier.srcAccessMask	= 0; // TODO
	barrier.dstAccessMask	= 0; // TODO

	// Transition barrier mask rule setup
	VkPipelineStageFlags sourceStage, destinationStage;
	{
				 ...
				 newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
		{
			...
			// Earliest access for pre-barrier operations
			sourceStage			= VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
			// Pseudo-stage for transfers
			destinationStage	= VK_PIPELINE_STAGE_TRANSFER_BIT;
			// For more: https://docs.vulkan.org/spec/latest/chapters/synchronization.html#VkPipelineStageFlagBits
		}
		...
		else // Unsupported layout transition
		{
			throw std::invalid_argument("[ERROR] : unsupported layout transition!");
		}
		// VK_IMAGE_LAYOUT_GENERAL layout supports all operations,
		// but at the cost of performance
	}

	vkCmdPipelineBarrier(
		commandBuffer,
		// Define which pipeline stages to operate on before and after the barrier
		// values depend on the access operation
		// for more: https://www.khronos.org/registry/vulkan/specs/1.3-extensions/html/chap7.html#synchronization-access-types-supported
		sourceStage			/* TODO srcStageMask */, 
		destinationStage	/* TODO dstStageMask */,
		// Depencies for execution and memory usage
		// VK_DEPENDENCY_BY_REGION_BIT	: turns barrier into per-region condition 
		//								  (allows reading from already modified parts)
		0,
		0,	nullptr,	/* Lenght and pointer to VkMemoryBarrier array structure */
		0,	nullptr,	/* Lenght and pointer to VkBufferMemoryBarrier array structure */
		1,	&barrier	/* Lenght and pointer to VkImageMemoryBarrier array structure */
	);

	endSingleTimeCommands(commandBuffer);
}

void HelloTriangleApp::endSingleTimeCommands(
	VkCommandBuffer commandBuffer
)
{
